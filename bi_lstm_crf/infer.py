...
BiLSTM-CRF Model Implementation

This script provides inference capabilities for a pre-trained BiLSTM-CRF model
...
class BiLSTMCRFInference:
    """
    Vietnamese NER inference class using BiLSTM-CRF model
    ...
    def __init__(self, model_path: str, device: str = 'auto'):
        """
        Initialize BiLSTM-CRF inference with trained model
        ...
        # Vietnamese text processing patterns
        self.vietnamese_chars = re.compile(r'[àáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ]', re.IGNORECASE)
    ...
    def _load_model(self):
        ...
        try:
            ...
            self.model = BiLSTM_CRF_NER.load(str(self.model_path))
            ...
            print(f"Available entity types: {[tag for tag in self.tag_vocab.get_itos() if tag not in [BOS, EOS, PAD, 'O']]}")
    ...
    def preprocess_tokens(self, tokens: List[str]) -> Tuple[torch.Tensor, List[int]]:
        ...
        # Add BOS and EOS tokens as expected by the model
    ...
    def predict_sentence(self, tokens: List[str], confidence_threshold: float = 0.0) -> List[Tuple[str, str, float]]:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                predicted_tags = self.model.predict(token_tensor, sent_lengths)
                
                # Convert tag indices back to labels
                if predicted_tags and len(predicted_tags) > 0:
                    tag_sequence = predicted_tags[0]  # Get first (and only) sequence
                    
                    # Skip BOS and EOS tokens in the prediction
                    if len(tag_sequence) >= 2:
                        tag_sequence = tag_sequence[1:-1]  # Remove BOS and EOS
                    
                    # Convert indices to IOB format
                    predicted_labels = self.model.iob_tag(tag_sequence)
                    
                    # Ensure we have the same number of tokens and labels
                    min_len = min(len(tokens), len(predicted_labels))
                    
                    # For now, set confidence to 1.0 (CRF doesn't provide confidence scores easily)
                    results = []
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="BiLSTM-CRF Named Entity Recognition Inference for Vietnamese Text",
...
Examples:
  ...
  python infer.py --text "Bệnh nhân Nguyễn Văn A, 35 tuổi." --confidence 0.8
    ...
    parser.add_argument('--model', '-m', type=str,
                       ...
                       help='Path to the trained BiLSTM-CRF model checkpoint (default: checkpoints/lstm_ner_10.pt)')
    ...
    try:
        ...
        print("Initializing BiLSTM-CRF NER model...")
        inferencer = BiLSTMCRFInference(args.model, args.device)

        if args.interactive:
            # Interactive mode
            print("\n" + "="*60)
            print("BiLSTM-CRF NER Interactive Mode")
